...
### 2.1 Actors
- **BC Оркестрация проверок качества (Quality Gate Orchestration BC):** инициирует запуск required набора проверок.
- **Разработчик (Developer):** потребляет результаты через отчетность и исправляет найденные нарушения.
...
- **Оркестрация проверок качества (Quality Gate Orchestration):** источник плана required checks.
- **Отчетность и доказательная база (Reporting and Evidence):** приемник результатов, логов и findings.
- **Система непрерывной интеграции (Continuous Integration System):** среда исполнения job-ов; в UC-VP-01 не моделируется как отдельный actor sequence-диаграммы.
...
**Триггер:** команда запуска набора checks от Оркестрация проверок качества (Quality Gate Orchestration).  
...
**Постусловия:** результаты и логи отправлены в Отчетность и доказательная база (Reporting and Evidence).

### 5.1 Lanes
- **ACTOR:** BC Оркестрация проверок качества (Quality Gate Orchestration BC)
- **BC:** BC Пакеты проверок (Verification Packages BC)
- **EXT:** Оркестрация проверок качества (Quality Gate Orchestration), Отчетность и доказательная база (Reporting and Evidence)

### 5.2 Основная последовательность (Happy Path)
1. Оркестрация проверок качества (Quality Gate Orchestration) -> **(CMD) StartRequiredVerificationSet** -> BC Пакеты проверок (Verification Packages BC).
...
9. BC Пакеты проверок (Verification Packages BC) -> **(CMD) PublishVerificationResults** -> Отчетность и доказательная база (Reporting and Evidence).
10. BC Пакеты проверок (Verification Packages BC) -> **(EVT) VerificationSetPublished** -> Оркестрация проверок качества (Quality Gate Orchestration).
...
- **Ключевые поля payload:**
  ...
  - `repository_slug`: идентификатор репозитория-источника run.
...
#### UC-VP-01A: Обнаружено нарушение безопасности (security finding)
...
2. BC Пакеты проверок (Verification Packages BC) -> **(CMD) MarkVerificationSetFailed** -> Оркестрация проверок качества (Quality Gate Orchestration).
3. BC Пакеты проверок (Verification Packages BC) -> **(CMD) PublishSecurityEvidence** -> Отчетность и доказательная база (Reporting and Evidence).
...
![Диаграмма последовательности: UC-01 / UC-VP-01 Выполнение базового набора проверок (Execute Baseline Verification Set)](diagrams/UC-01-sequence.png)
