...
- **Оркестрация проверок качества (Quality Gate Orchestration):** источник итогового статуса gate-run.
- **Пакеты проверок (Verification Packages):** источник детальных результатов и логов.
...
**Триггер:** завершение run в Оркестрация проверок качества (Quality Gate Orchestration).  
...
- **EXT:** Оркестрация проверок качества (Quality Gate Orchestration), Пакеты проверок (Verification Packages), GitHub Интерфейс PR / хранилище артефактов (PR UI / Artifact Storage)

### 5.2 Основная последовательность (Happy Path)
1. Оркестрация проверок качества (Quality Gate Orchestration) -> **(EVT) GateRunCompleted** -> BC Отчетность и доказательная база (Reporting and Evidence BC).
2. Пакеты проверок (Verification Packages) -> **(EVT) VerificationDetailsAvailable** -> BC Отчетность и доказательная база (Reporting and Evidence BC).
...
#### UC-RE-01A: Отсутствует лог у упавшей проверки (failed check)
...
3. BC Отчетность и доказательная база (Reporting and Evidence BC) -> **(CMD) RequestLogReupload** -> Пакеты проверок (Verification Packages).
