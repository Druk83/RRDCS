...
- **Оркестрация проверок качества (Quality Gate Orchestration):** получает required checks и пороги.
- **Пакеты проверок (Verification Packages):** получает baseline-параметры.
- **Git-репозиторий и поток PR (Git Repository and Pull Request Flow):** канал versioned изменений policy.
- **Отчетность и доказательная база (Reporting and Evidence):** получает события onboarding и rollout-режима репозитория.
- Первый rollout начинается с одного pilot-репозитория; `wave/global` подключаются после стабилизации pilot.
...
- **EXT:** Git-репозиторий и поток PR (Git Repository and Pull Request Flow), Оркестрация проверок качества (Quality Gate Orchestration), Пакеты проверок (Verification Packages)
...
7. BC Управление политиками и цепочкой инструментов (Policy and Toolchain Governance BC) -> **(CMD) PublishPolicyToOrchestrator** -> Оркестрация проверок качества (Quality Gate Orchestration).
8. BC Управление политиками и цепочкой инструментов (Policy and Toolchain Governance BC) -> **(CMD) PublishBaselineToVerificationPackages** -> Пакеты проверок (Verification Packages).
...
#### UC-PG-01A: Изменение отклонено на этапе governance (governance)
...
3. BC Управление политиками и цепочкой инструментов (Policy and Toolchain Governance BC) -> **(CMD) PublishProfileToOrchestrator** -> Оркестрация проверок качества (Quality Gate Orchestration).
4. BC Управление политиками и цепочкой инструментов (Policy and Toolchain Governance BC) -> **(EVT) RepositoryOnboardedInAuditMode** -> Git-репозиторий и поток PR (Git Repository and Pull Request Flow), Отчетность и доказательная база (Reporting and Evidence).
...
6. BC Управление политиками и цепочкой инструментов (Policy and Toolchain Governance BC) -> **(EVT) RepositoryPromotedToRequired** -> Оркестрация проверок качества (Quality Gate Orchestration), Отчетность и доказательная база (Reporting and Evidence).
...
![Диаграмма последовательности: UC-02 / UC-PG-02 Подключение репозитория и staged rollout (pilot first)](diagrams/UC-02-sequence.png)

## 11. Операционный runbook
- `repository-onboarding-runbook.md` — пошаговая инструкция по подключению репозитория и переводу `audit -> required`; первый rollout выполняется на одном pilot-репозитории.
