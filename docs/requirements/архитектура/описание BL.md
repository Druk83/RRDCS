...
| BO-007 | Профили интеграции репозиториев (Repository Integration Profiles) | Правила onboarding и rollout для конкретных репозиториев |
...
- `BR-BL-05`: onboarding репозитория выполняется через `Repository Integration Profile` с контролируемым переходом `audit -> required`.

## 6. Трассировка BL к требованиям
| Процесс | FR | NFR |
|---|---|---|
| BP-001 | FR-005 | NFR-004, NFR-007 |
| BP-002 | FR-001, FR-002, FR-003, FR-004, FR-010, FR-013 | NFR-001, NFR-005 |
| BP-003 | FR-006, FR-007, FR-009 | NFR-003, NFR-004 |
| BP-004 | FR-008 | NFR-002, NFR-006 |
| BP-005 | FR-011, FR-012, FR-013 | NFR-009 |
...
## 8. Межслойные связи (зеркальная фиксация AL -> BL)
